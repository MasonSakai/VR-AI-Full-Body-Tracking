...
IDashboardListener* listener = NULL;
vr::IVRSystem* vrSystem; //Are these even needed?
...
VRDashboardOverlay* VRDashboardOverlay::SharedInstance()
{
	...
	{
		throw std::exception("VRDashboardOverlay: No shared instance created");
	}
	return s_pSharedVRController;
}

VRDashboardOverlay* VRDashboardOverlay::CreateSharedInstance(IDashboardListener *dashboardListener, int argc, char* argv[]) {

	if (!s_pSharedVRController)
	{
		listener = dashboardListener;
		//vrSystem = dashboardListener->GetVRSystem();
		//vrOverlay = dashboardListener->GetVROverlay();
		s_pSharedVRController = new VRDashboardOverlay(argc, argv);
	}
	return s_pSharedVRController;
}
	...
	, m_bManualMouseHandling(false)
{
	...
	DashboardWidget* pOverlayWidget = new DashboardWidget(listener);
...
bool VRDashboardOverlay::ConnectToVRRuntime()
{
	m_eLastHmdError = vr::VRInitError_None;
